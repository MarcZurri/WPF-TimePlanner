...
{
    internal class PlannerWeek : PlannerBase
    {
        ...
        internal PlannerWeek()
        {
        }

        protected override void CalculateCurrentPeriodStartDate()
        {
            this.CurrentPeriodStartDate = this.FocusDate.AddDays(-(int)this.FocusDate.DayOfWeek);

            if (CultureInfo.CurrentCulture.DateTimeFormat.FirstDayOfWeek != DayOfWeek.Sunday)
                this.CurrentPeriodStartDate = this.CurrentPeriodStartDate.AddDays(1);
        }
